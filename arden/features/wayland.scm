(define-module (arden features wayland)
  ...
  #:use-module (arden utils bemenu-prompt)
  #:use-module (arden systems)
  #:use-module (arden packages wayland)
  ...
  #:export (
            ...
            %arden-dwl-guile-patches
            %arden-dwl-guile-config))

(define %arden-dwl-guile-patches
  (list %patch-xwayland
        %patch-swallow
        %patch-movestack
        %patch-attachabove))

(define %arden-dwl-guile-config
  (dwl-config
   (xkb-rules %arden-keyboard-layout)
   (border-px 2)
   (rules
    (list
     (dwl-rule (id "emacs")
